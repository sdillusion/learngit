...
void CC5Modbus::RequestData()
{
	...
	{
	case YC_CYCLIC:	m_askFrameType = YX_CYCLIC; RequestYx(); break;
	case YX_CYCLIC:	m_askFrameType = DD_CYCLIC; RequestDD(); break;
	case DD_CYCLIC:	m_askFrameType = YC_CYCLIC; RequestYc(); break;
...
bool CC5Modbus::IsNeedToSendHeartBeat(int now){
	...
	//if(now - m_lastReqTime > gheartbeatInterval  && now - m_lastHertTime > gheartbeatInterval){
	//	return true;
	//}
